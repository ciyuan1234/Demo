...
async function handle(request: Request, env: Env): Promise<Response> {
  ...
  if (deviceMatch) { const row = await env.DB.prepare(`SELECT device_id,pond_id,mode,status FROM devices WHERE device_id=?`).bind(deviceMatch[1]).first(); return row ? json(row) : json({ detail: 'device not found' }, 404) }
